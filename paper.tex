...
Among the useful properties of this factorization, is that the product $US_k\trans{V}$ is the best rank-$k$ approximation of $X$ under the Frobenius matrix norm, where $S_k$ is the matrix $S$ with all but the first $k$ diagonal entries zeroed out.
The $k$ initial coordinates of the rows of $US_k$ can thus be viewed as an optimal $k$-dimensional reduced model of the rows of $X$, with a new coordinate system given by the columns of $V$.
