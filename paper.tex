...
The essential idea of this algorithm is to compute the number of bits needed to encode the data under various assumed groupings and their implied distributions.
The grouping which leads to the least overall encode size is the desired segmentation.
This algorithm has many desirable qualities:
unlike most clustering algorithms, it finds subspaces, not full-rank regions;
it is highly robust to outliers and noisy data.
This algorithm has one parameter:
the maximum allowable distortion of the data, $\varepsilon^2$.
We follow Ma's lead when applying their algorithm to \caps{SVD} data and use $\varepsilon^2 = 1$.
...
We have determined, as well as we can, which of the $q_i$ belong to the same structures.
Now we must determine the distributions $p_j$ which generate these structures.
...
many invalid points will lie within the convex hull of these vectors.
\caps{SVD} can give orthogonal vectors spanning the structure, but we must turn to other techniques to recover possible vectors for the $p_j$.
...
$P$ is then a vertical concatenation of small $P_*$ matrices, while $W$ is a direct sum of $W_*$ matrices.
Many \caps{NMF} algorithms have been proposed since Lee and Seung published the first~\cite{Lee01}.
We use Kim and Park's alternating non-negative least squares (\caps{ANLS}) algorithm~\cite{Kim08}.
It converges much more rapidly than the Lee and Seung algorithm, but stalls when it gets close to an optimum, so we refine that result by applying the Lee and Seung algorithm.
...
We find that using standard random \caps{NMF} initialization does not yield factorizations of sufficient quality, so we have developed the following initialization procedure.
...
These rows provide a ``denoised'' set of nonnegative spanning vectors that should recover the true low-dimensional subspace from which the data were sampled.
Finally, we find the intersection of this subspace with the standard simplex in $\R^n$.
This step gives us a set of vertices geometrically guaranteed to contain all of the recovered low-dimensional subspace.\cddnote
