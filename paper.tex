...
The essential observation of this research is that real traffic lies near the union of a small set of low rank subspaces in the representation space.
This allows us to perform a drastic model reduction on traffic data while preserving most characteristics of the original traffic.
Moreover, when observable characteristics of flows are separated from features to be predicted, this model reduction is similar to a classical linear regression, and allows us to very simply predict unseen behavior of flows.
We apply this technique here to directly predict the following characteristics of each flow from observing only a handful of its initial packets:
\begin{enumerate}
