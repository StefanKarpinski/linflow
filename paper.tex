...
The K-S test compares the cumulative distribution function (\caps{CDF}) of a theoretical distribution with the empirical cumulative distribution function of a sample of values, such as packet sizes.
...
the probability of obtaining a result at least as extreme as the observed result, assuming that the sample did come from the theoretical distribution.
...
We evaluate the quality of our packet size and inter-packet interval distribution predictions by looking at the distribution of K-S p-values for the predicted distribution versus the observed sample.
...
Instead of comparing predictions to the theoretical ideal of uniformity, we use a more pragmatic ideal as a baseline:
...
\Figure{cdf} shows the the \caps{CDF}s p-values for our ideal baseline and the predicted packet size and inter-packet interval distributions.
For comparison, we also show the p-values results for two simplistic prediction techniques.
The first alternative prediction technique is to always predicting the mean behavior of the entire training trace.
This does not perform as badly as one might think it would---always guessing the average is not an abysmal failure.
The second alternative technique is to just guess what you've already seen.
We call this ``self-prediction.''
This is likely the best guess one can make about a flow, without having learned from any other flows:
