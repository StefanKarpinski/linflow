...
Yet the properties of the model explain the structures seen in \Figure{svd}.
However, this is only one hypothesis that could fit the data.
Like any hypothesis, it must be tested.
The rest of this paper, aside from providing a useful, practical application, serves as a hypothesis test:
we try to recover the matrices $W$ and $P$ from our imperfect observations of $Q$ and use the recovered model to predict flow behaviors.
If the recovered model makes accurate predictions, this provides evidence that our hypothesis is correct.
