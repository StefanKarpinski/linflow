...
A linear representation, as defined by Karpinski~\emph{et~al.}~\cite{Karpinski08}, maps sets of flows into a vector space such that union of sets becomes summation in the vector space:
\begin{align}
  \Phi(s_1 \union s_2) = \Phi(s_1) + \Phi(s_2).
\end{align}
The representation function, $\Phi$, should map each set of flows to a vector which in some sense describes their collective behavior.
For example: let $\Phi(s) \in \R^1$ be the number of packets in all the flows in the set $s$.
Since the number of total packets in the union of two sets of flows is the sum of the number in each individually, this representation is linear.
...
As mentioned before, a 1500-dimensional vector can be used to count the frequency of all possible packets sizes from a single byte up to the typical \caps{MTU} of 1500 bytes.
In this representation, $\Phi(s) \in \R^{1500}$ has as its $i$th entry the number of packets in $s$ of size $i$.
Since \caps{IP} protocol types range from 0 to 255, the frequency of types in a set of flows can be represented in 256 dimensions.
The representation details for port numbers and inter-packet intervals are described detail in \Section{methodology}.
...
The representation we consider here is a direct sum of such histogram representations.
